 ...
 * @description 🐟每天都是摸鱼人。
 * @version v1.0.0
 * @icon https://pic1.zhimg.com/80/v2-fe17444b7c57c68b100d64de3ffa546e_1440w.jpg?source=1940ef5c
 */
...
var isOnline = true
// 黄历群白名单（置空则都展示）
var whiteList = [1044010101, 18843026371]
// 摸鱼表情包，随机展示
var fishPic = [
...
function main() {
    ...
    if (whiteList.length == 0 || whiteList.indexOf(s.getChatId()) >= 0 || (s.getUserId() ? "是" : "不是")) {
        dateApi();
    }
    if (fishPic.length != 0) {
...
function dateApi() {
    ...
    try {
        ...
        msg += "\n————  ┑(￣Д ￣)┍  ————"
        msg += "\n【今日黄历】" + req.body["yearname"] + "年 " + req.body["nonglicn"] + " " + req.body["jieqi"]
        msg += "\n「宜」" + req.body["suit"].replace(/\./g, "·")
        msg += "\n「忌」" + req.body["avoid"].replace(/\./g, "·")
    } catch (e) { }
}
